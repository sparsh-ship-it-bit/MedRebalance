    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {loading ? (
          ...
          <>
            <MetricCard icon={<IndianRupee className="w-6 h-6" />} label="Total Waste Prevented" value={`₹${totalWastePrevented.toLocaleString('en-IN')}`} subtitle={`${scopedTransactions.length} completed transfers`} gradient="from-emerald-500 to-teal-600" />
            <MetricCard icon={<Boxes className="w-6 h-6" />} label="Active Units Rebalanced" value={activeUnitsRebalanced.toLocaleString('en-IN')} subtitle={`${activeTransfers.length} in transit`} gradient="from-blue-500 to-cyan-600" />
            <MetricCard icon={<TrendingUp className="w-6 h-6" />} label="Platform Revenue Earned" value={`₹${platformRevenue.toLocaleString('en-IN')}`} subtitle="3.5% fee per transaction" gradient="from-amber-500 to-orange-600" />
          </>
        )}
      </div>
...
function MetricCard({ icon, label, value, subtitle, gradient }: { icon: React.ReactNode; label: string; value: string; subtitle: string; gradient: string }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${gradient} p-5 shadow-lg`}>
      ...
      <div className="relative">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-white/20 backdrop-blur-sm mb-3 text-white">{icon}</div>
