...
async function readError(response: Response): Promise<string> {
  ...
  } catch { return `Supabase request failed (${response.status})`; }
}
...
router.post("/register-hospital", async (req, res) => {
  ...
  const validCoords = Number.isFinite(Number(lat)) && Number.isFinite(Number(lng));
  if (!hospitalName?.trim() || !address?.trim() || !city || !hospitalType || !normalizedEmail || !validCoords || typeof password !== "string" || password.length < 6) {
    res.status(400).json({ error: "Please provide valid hospital details, email, coordinates, and a password of at least 6 characters." });
    return;
  }
  ...
  try {
    ...
    if (!authResponse.ok) {
      ...
      res.status(authResponse.status === 422 || authResponse.status === 400 ? 400 : authResponse.status).json({
        error: duplicate ? "This email is already registered. Sign in instead, or use a different email address." : message,
      });
      return;
    }
    ...
    const hospitalResponse = await supabaseRequest("/rest/v1/hospitals", {
      ...
      body: JSON.stringify({ name: hospitalName.trim(), address: `${address.trim()}, ${city}`, lat: Number(lat), lng: Number(lng), type: hospitalType }),
    });
    ...
    if (userId) { try { await supabaseRequest(`/auth/v1/admin/users/${userId}`, { method: "DELETE" }); } catch { /* best effort */ } }
    if (hospitalId) { try { await supabaseRequest(`/rest/v1/hospitals?id=eq.${encodeURIComponent(hospitalId)}`, { method: "DELETE" }); } catch { /* best effort */ } }
